...
// item, its files ordered by disc - unless their serials differ (one line per regional release): then each
// serial is an item of its own, and its id gains "/<serial>". version/serial/image/description come from
// whichever line has them. A line without a title or an http(s) url is skipped, and said so in `problems` -
...
// size, "Title ID" the serial, and "Name" the title when the header has no "title" column. Its links are PSN
