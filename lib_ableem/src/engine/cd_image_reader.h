...
public:
    ...
    void allocateBuffer() { buffer.assign(CD_FRAME_SIZE, 0); }
    char *getBuffer() { return buffer.data(); }
    int getSectorPos() { return sectorpos; }
    void setSectorpos(int pos) { sectorpos = pos; }

    void setCurrentSector(int pos) { currentSector = pos; }
    ...
    virtual bool endStream() { return stream.tellg() == -1; }
};
...
private:
    ...
    bool readTrackMetadata() {
        ...
        if (chd_get_metadata(chd, CDROM_TRACK_METADATA2_TAG, 0, metadata, sizeof(metadata), &metalen, nullptr,
                             nullptr) == CHDERR_NONE &&
            sscanf(metadata, CDROM_TRACK_METADATA2_FORMAT, &tracknum, type, subtype, &frames, &pregap, pgtype, pgsub,
                   &postgap) >= 4) {
            trackFrames = frames;
            return true;
        }
        if (chd_get_metadata(chd, CDROM_TRACK_METADATA_TAG, 0, metadata, sizeof(metadata), &metalen, nullptr,
                             nullptr) == CHDERR_NONE &&
